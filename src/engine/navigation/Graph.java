...
public class Graph<V> {
		...
		// of the vertex's connected to the added vertex
			...
			for (Edge<V> vertexConnectedToAddedVertex : connectedVertices) {
				...
				if (!directed) {
					...
					// graph is equal
					float weight = vertexConnectedToAddedVertex.getWeight();
	...
	public void remove(V vertex) {
		...
		Edge<V> edgeToBeRemoved = null;
		for (V v : getVertexList()) {
	...
	public boolean addArc(V source, V end, float weight) {
		if (!directed) {
			return false;
		}
	...
	public boolean addEdge(V vertexOne, V vertexTwo, float weight) {
		if (directed) {
			return false;
		}
	...
	public V getVertexList(int index) {
